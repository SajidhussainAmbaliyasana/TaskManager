...
export default function CreateTaskDialog({ open, defaultStage = 'Todo', onClose, onSave }) {
  const [form, setForm] = useState({ title: '', description: '', stage: defaultStage, priority: 'Medium', tags: [] });
  const [tagInput, setTagInput] = useState('');
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (open) setForm(f => ({ ...f, stage: defaultStage }));
  }, [open, defaultStage]);
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    await onSave({ title: form.title, description: form.description, stage: form.stage, priority: form.priority, tags: form.tags });
    setLoading(false);
    setForm({ title: '', description: '', stage: defaultStage, priority: 'Medium', tags: [] });
    onClose();
  };
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle sx={{ pb: 1 }}>
        <Typography variant="h6" sx={{ color: '#F8FAFC', fontFamily: '"Syne", sans-serif', fontWeight: 700 }}>Create new task</Typography>
        <Typography variant="caption" sx={{ color: '#64748B' }}>Add a task to your board</Typography>
      </DialogTitle>

      <DialogContent sx={{ pt: 2 }}>
        <Stack spacing={2.5}>
          <TextField label="Task title" fullWidth value={form.title} onChange={set('title')} placeholder="e.g. Design landing page mockup" InputLabelProps={{ sx: { color: '#64748B' } }} autoFocus />
          <TextField label="Description" fullWidth multiline rows={3} value={form.description} onChange={set('description')} placeholder="Describe the task in detail..." InputLabelProps={{ sx: { color: '#64748B' } }} />
      ...
      <DialogActions sx={{ px: 3, pb: 3, pt: 1, gap: 1 }}>
        ...
        <Button variant="contained" onClick={handleSave} disabled={!form.title.trim() || !form.description.trim() || loading}>
          {loading ? <CircularProgress size={18} sx={{ color: '#fff' }} /> : 'Create task'}
        </Button>
      </DialogActions>
    </Dialog>
  );
